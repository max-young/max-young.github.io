...
实现多进程我们也可以使用multiprocessing.pool, 示例代码如下:
```python
from multiprocessing import Pool
# Pool的参数6代表worker数量, 根据机器和实际情况来调试, 看配置多大能达到最大效率
# record_upsert是要执行的函数, 后面的list是参数
with Pool(6) as p:
    p.starmap(record_upsert, [(r, override) for r in rs["data"]])
```
在这里, 我们同样做不到共享数据, 因为是在多个进程里执行, 那么我们我们如何让多个进程里的数据联系到一起呢
...
print("init list is {}".format(my_list))
loop = asyncio.get_event_loop()
loop.run_until_complete(foo(my_list))
loop.close()
end = time.time()
print("finally list is {}".format(my_list))
print("运行时间是: {}".format(end-start))
...
运行结果是:
```shell
main thread is <_MainThread(MainThread, started 140736192115584)>
init list is []
foo start
foo thread is <_MainThread(MainThread, started 140736192115584)>
my_func thread is <_MainThread(MainThread, started 140736192115584)>
my_func thread is <_MainThread(MainThread, started 140736192115584)>
my_func thread is <_MainThread(MainThread, started 140736192115584)>
my_func thread is <_MainThread(MainThread, started 140736192115584)>
my_func thread is <_MainThread(MainThread, started 140736192115584)>
...
但是我们信息的看到: `results: [4, 9, 5, 1, 6, 2, 7, 3, 8, 0]`, 我们通过`completed, pending = await asyncio.wait(tasks)`可以将多个进程里执行的任务的结果组织起来, 这样建立了联系, 在某些场景里是很有用的.
