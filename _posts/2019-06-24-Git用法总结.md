...
1. 开发 
  ...
  - 联调和开发测试均在此分支上  
    ...
    - 在本地构建镜像
    ```shell
    $ docker build . -t registry.cn-beijing.aliyuncs.com/myproject/clm:dev
    $ docker push registry.cn-beijing.aliyuncs.com/myproject/clm
    ```
    - 在测试服务器上上线  
    ```shell
    $ docker pull registry.cn-beijing.aliyuncs.com/myproject/clm:dev && docker-compose -f /var/www/devops/docker-compose-test.yml up -d
    ```
